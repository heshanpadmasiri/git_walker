...
fn execute_action(action: Action) -> Result<(), String> {
    match action {
        ...
        Action::Atomize(TestCommand {start, end, command, path}) => utils::execute_atomize(&path, &command, &start, &end),
        Action::Help => {
            print_help();
            Ok(())
        }
    ...
    match args.as_slice() {
        [path, start_commit, end_commit, remaining_args @ ..] => {
            ...
            match command.as_str() {
                ...
                "atomize" => finish_parsing_atomize_args(remaining_args, path, start_commit, end_commit),
...
fn finish_parsing_atomize_args(
    ...
    Ok(Args {
        action: Action::Atomize(TestCommand {
